...
public class TSAnalyzer {
	...
	private final Map<TSAnalyzer.annotations,Statistics> statistics;
	...
	public TSAnalyzer(final PluginContext context, final EventPayloadTransitionSystem ts, final XLog log) {
		...
		this.statistics = new HashMap<TSAnalyzer.annotations,Statistics>();
	...
	private void processFrequencyEvent(final XTrace pi) {
		...
		Map<State,Byte> stateObservations = new HashMap<State,Byte>();
		Map<Transition,Byte> transitionObservations = new HashMap<Transition,Byte>();
		
		//initialize with 0s
		
		for(State s : transitionSystem.getNodes())
			stateObservations.put(s, (byte) 0);
		
		for(Transition t : transitionSystem.getEdges())
			transitionObservations.put(t, (byte) 0);
			...
			if (transition != null) 
			{
				...
				if(i == 0) //annotate the source only for the first event, this avoids double-counting states
				{
					if(stateObservations.containsKey(source))
						stateObservations.put(source, (byte) (stateObservations.get(source) + 1)); //adds 1 to the observations of the source state
					else
						stateObservations.put(source, (byte) 1);
				}
				//annotate target states
				if(stateObservations.containsKey(target))
					stateObservations.put(target, (byte) (stateObservations.get(target) + 1)); //adds 1 to the observations of the target state
				else
					stateObservations.put(target, (byte) 1);
				
				//annotate transitions
				if(transitionObservations.containsKey(transition))
					transitionObservations.put(transition, (byte) (transitionObservations.get(transition) + 1)); //adds 1 to the observations of the transition
				else
					transitionObservations.put(transition, (byte) 1);
			}
		}
