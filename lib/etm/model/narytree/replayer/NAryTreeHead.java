...
public class NAryTreeHead implements TreeHead {
	...
	public Head getNextHead(Record rec, Delegate<? extends Head, ? extends Tail> d, int first, TIntCollection modelMoves) {
		...
		synchronized (delegate) {
			if (delegate.useQueue()) {
				assert delegate.getQueue().isEmpty();
				delegate.getQueue().add(first);
				TIntIterator it = modelMoves.iterator();
				while (it.hasNext()) {
					delegate.getQueue().add(it.next());
				}
				while (!delegate.getQueue().isEmpty()) {
					int node = delegate.getQueue().poll();
					if (node != first) {
					...
					newState = delegate.execute(newState, node, delegate.getQueue());
				}
			} else {
				newState = delegate.execute(newState, first);
				TIntIterator it = modelMoves.iterator();
				while (it.hasNext()) {
					newState = delegate.execute(newState, it.next());
				}
			}
		}
	...
	public Head getNextHead(Record rec, Delegate<? extends Head, ? extends Tail> d, int modelMove, int logMove,
		...
		if (modelMove != AStarThread.NOMOVE) {
			...
			synchronized (delegate) {
				if (delegate.useQueue()) {
					assert delegate.getQueue().isEmpty();
					delegate.getQueue().add(modelMove);
					while (!delegate.getQueue().isEmpty()) {
						int node = delegate.getQueue().poll();
						if (node != modelMove) {
							newMovesMade.add(node);
					...
					newState = delegate.execute(newState, modelMove);
				}
			}
		}
		return new NAryTreeHead(newParikh, newState, hash(newState, newParikh), newMovesMade, newMovesMadeCost);
