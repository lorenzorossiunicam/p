...
public class DottedChartPanel extends JPanel implements MouseListener, MouseMotionListener {
	...
	public static String HANDLE_ICONS[] = { STR_NONE, ITEM_HANDLE_CIRCLE, ITEM_HANDLE_TRIANGLE, ITEM_HANDLE_BOX,
			ITEM_HANDLE_RHOMBUS };
	...
	private final int dismissDelay = 1000 * 60 * 5;
	...
	public void setViewportZoomIn() {
		...
		int value = (int) (Math.log10(getWidth() * (d.getWidth() / width) / dca.getViewportSize().getWidth()) * 1000.0);
		if (value > 3000) {
			return;
		}
		value = (int) (Math.log10(getHeight() * (d.getHeight() / height) / dca.getViewportSize().getHeight()) * 1000.0);
		if (value > 3000) {
			return;
		}

		updWidth = (int) (getWidth() * (d.getWidth() / width));
		updHight = (int) (getHeight() * (d.getHeight() / height));
		Dimension dim = new Dimension(updWidth, updHight);
	...
	public void adjustWidth() {
		...
		if (number == 0) {
			...
			if (idx >= widthDividers.length) {
				dca.getDottedChartOptionPanel().changeWidthSort(widthDividers.length - 1);
			} else {
				dca.getDottedChartOptionPanel().changeWidthSort(idx);
			}
		}
	...
	public void paintComponent(Graphics grx) {
		...
		if (modelBuffer == null) {
			setToolTipText(null);
			if (getWidth() <= SCREENLENGTH) {
				coUtil.setClipL(0);
				coUtil.setClipR(getWidth());
			} else {
				...
				} else if (x2 + SCREENLENGTH >= getWidth()) {
					coUtil.setClipR(getWidth());
					coUtil.setClipL(getWidth() - SCREENLENGTH);
				} else {
					coUtil.setClipL(x2);
					coUtil.setClipR(x2 + SCREENLENGTH);
				}
			}
			if (getHeight() <= SCREENLENGTH) {
				coUtil.setClipU(0);
				coUtil.setClipB(getHeight());
			} else {
				...
				} else if (y2 + SCREENLENGTH >= getHeight()) {
					coUtil.setClipU(getHeight() - SCREENLENGTH);
					coUtil.setClipB(getHeight());
				} else {
					coUtil.setClipU(y2);
	...
	protected long getLeftRight(int k) {
		if (dcop.getTimeOption().equals(DottedChartModel.TIME_ACTUAL)) {
			return ((coUtil.coord2timeMillis(k) + timeOffset) / dcModel.getProperTimeUnit(dcop.getTimeOption(),
					coUtil.getMilli2pixels()));
		}
		if (dcop.getTimeOption().equals(DottedChartModel.TIME_RELATIVE_RATIO)
				|| dcop.getTimeOption().equals(DottedChartModel.TIME_RELATIVE_TIME)) {
			return ((coUtil.coord2timeMillis(k)) / dcModel.getProperTimeUnit(dcop.getTimeOption(),
					coUtil.getMilli2pixels()));
		}
	...
	protected void drawModelBuffer(int imageWidth, int imageHeight, boolean isModel) {
		...
		HashMap<String, HashMap<String, ArrayList<Integer>>> timeMap = dcModel.getProperTimeMap(dcop.getTimeOption()
		...
		if (dcop.getSortStandard().equals(DottedChartModel.STR_NONE)) {
			for (long j = left; j <= right; j++) {
				...
				for (String str : tempMap.keySet()) {
					currentTop = coUtil.unit2CordHeight(Integer.valueOf(str), compNumber, imageHeight, tempBorder)
							+ unitHeight / 2;
					...
					if (isModel) {
						uiUtil.paintItemHigh(coUtil.time2coord(temp) + tempBorder - coUtil.getClipL(), currentTop
								- upper, gBuf, ITEM_HANDLE_CIRCLE, tempMap.get(str).size());
					} else {
						uiUtil.paintItem_buffer(coUtil.time2coordOverview(temp, milli2pixelsBuffer) + tempBorder,
								currentTop, gBuf, ITEM_HANDLE_CIRCLE, tempMap.get(str).size());
					}

				}
			}
		} else {
			HashMap<Integer, Integer> mapping = dcModel.getSortedMapModel().getSortedMapping(dcop.getTimeOption(),
			...
			for (long j = left; j <= right; j++) {
				...
				for (String str : tempMap.keySet()) {
					currentTop = coUtil.unit2CordHeight(mapping.get((int) Integer.valueOf(str)), compNumber,
							imageHeight, tempBorder) + unitHeight / 2;
					if ((currentTop < upper) || (currentTop > bottom)) {
						continue;
					}
					assignColorByItem(tempMap.get(str), gBuf);
	...
	protected void drawModelBuffer_NO_SIZE(int imageWidth, int imageHeight, boolean isModel) {
		...
		if (isModel) {
			...
			clipRightTs = coUtil.coord2timeMillis(getWidth());
			upper = 0;
			bottom = getHeight();
		}
		...
		for (int k = 0; k < items.length; k++) {
			...
			if (isModel) {
				uiUtil.paintItem(coUtil.time2coord(items[k]) + tempBorder - coUtil.getClipL(), currentTop - upper,
						gBuf, assignShapeByItem(k));
			} else {
				uiUtil.paintItem_buffer(coUtil.time2coordOverview(items[k], milli2pixelsBuffer) + tempBorder,
						currentTop, gBuf, assignShapeByItem(k));
			}
		}
	}
	...
	protected void paintComponentLane(Graphics g, int imageWidth, int imageHeight, boolean isModel) {
		...
		Iterator<String> itr = dcModel
				.getSortedMapModel()
				.getSortedItemArrayList(dcop.getTimeOption(), dcop.getComponentType(), dcop.getSortStandard(),
	...
	private String hasDots_NO_SIZE(Point p1, Point p2) {
		...
		for (int k = 0; k < items.length; k++) {
			...
			currentTop = coUtil.unit2CordHeight(intCodes[k], size, getHeight(), BORDER) + unitHeight / 2;
			if ((currentTop > min_y - 5) && (currentTop < max_y + 5)) {

				assignColorByItem(k, gBuf);
				if (clicked) {
					uiUtil.paintItem(coUtil.time2coord(items[k]) + BORDER - coUtil.getClipL(), currentTop, gBuf,
							assignShapeByItem(k));
					continue;
				} else {
					uiUtil.paintItemExt(coUtil.time2coord(items[k]) + BORDER - coUtil.getClipL(), currentTop, gBuf,
							assignShapeByItem(k));
				}
	...
	private String hasDots(Point p1, Point p2) {
		...
		for (long j = left; j <= right; j++) {
			...
			for (String str : tempMap.keySet()) {
				...
				if (dcop.getSortStandard().equals(DottedChartModel.STR_NONE)) {
					currentTop = coUtil.unit2CordHeight(Integer.valueOf(str), size, getHeight(), BORDER) + unitHeight
							/ 2;
				} else {
					currentTop = coUtil.unit2CordHeight(mapping.get((int) Integer.valueOf(str)), size, getHeight(),
							BORDER) + unitHeight / 2;
				}
				if ((currentTop > min_y - diameter) && (currentTop < max_y + diameter)) {
					...
					if ((pointX >= min_x - diameter - BORDER) && (pointX <= max_x + diameter - BORDER)) {
						assignColorByItem(tempMap.get(str), gBuf);
						if (clicked) {
							uiUtil.paintItemHigh(coUtil.time2coord(temp) + BORDER - coUtil.getClipL(), currentTop,
									gBuf, ITEM_HANDLE_CIRCLE, tempMap.get(str).size());
							continue;
						} else {
							uiUtil.paintItemHighExt(coUtil.time2coord(temp) + BORDER - coUtil.getClipL(), currentTop,
									gBuf, ITEM_HANDLE_CIRCLE, tempMap.get(str).size());
						}
