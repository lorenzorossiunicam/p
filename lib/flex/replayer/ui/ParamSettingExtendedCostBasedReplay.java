 ...
 * @version Dec 1, 2009
 */
public class ParamSettingExtendedCostBasedReplay extends ParamSettingStep {
	...
	public ParamSettingExtendedCostBasedReplay(boolean isTestingMode) {
		super(isTestingMode);
	...
	private void initComponents(boolean isTestingMode) {
		...
		basicPanel.add(slickerFactory
				.createLabel("<html><h1>Configure cost</h1><p>Check appropriate options</p></html>"),
				"0, 0, 1, 0, l, t");
		...
		basicPanel.add(slickerFactory.createLabel("<html><h2>How would you like the conformance checking to be performed?</h2></html>"), "0, "
				+ String.valueOf(basicRowIndex) + ", 1, " + String.valueOf(basicRowIndex));
		basicRowIndex++;
		...
		if (!isTestingMode) {
			size = new double[][] { { 400, 350 },
					{ 45, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30 } };
		} else {
			size = new double[][] {
					{ 400, 350 },
		...
		advancedPanel.add(slickerFactory.createLabel("<html><h2>Max # instances</h2></html>"), "0, "
				+ String.valueOf(rowIndex));
		advancedPanel.add(maxExpInstSlider, "1, " + String.valueOf(rowIndex++));

		// skipping event cost
		skipEventSlider = slickerFactory.createNiceIntegerSlider("", 1, 50, DEFSKIPPEDEVENTCOST, Orientation.HORIZONTAL);
		...
		noSkipEvent.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (noInappropriateFiring.isSelected()){
					JOptionPane.showMessageDialog(null, "Either \"Identifying inserted activities\" or \"Allowing violating sync log+model\" must be allowed", "Alert", JOptionPane.ERROR_MESSAGE);
		...
		// initiated execute invi task
		execInviTaskSlider = slickerFactory.createNiceIntegerSlider("", 0, 50, DEFINITIATIVEINVISTASKCOST, Orientation.HORIZONTAL);
		...
		// initiated execute real task
		execRealTaskSlider = slickerFactory.createNiceIntegerSlider("", 1, 50, DEFINITIATIVEREALTASKCOST, Orientation.HORIZONTAL);
		...
		unsatisfiedEventsSlider = slickerFactory.createNiceIntegerSlider("", 1, 50, DEFINAPPROPRIATETRANSFIRECOST, Orientation.HORIZONTAL);
		...
		advancedPanel.add(slickerFactory
				.createLabel("<html><h2>Cost of a Violating Log+Model Synchronous Activity</h2></html>"), "0, " + rowIndex
				+ ", 1, " + String.valueOf(rowIndex++));
		...
		if (isTestingMode) {
			...
			advancedPanel.add(slickerFactory.createLabel("<html><h2>Select testing type</h2></html>"), "0, "
					+ String.valueOf(rowIndex++));
			...
			advancedPanel.add(slickerFactory.createLabel("<html><h2>Output file location</h2></html>"), "0, "
					+ String.valueOf(rowIndex++));
	...
	public Object[] getAllParameters() {
		...
		if (useBasic) {
			parameters[ParamSettingExtendedCostBasedReplay.INAPPROPRIATETRANSFIRECOST] = DEFINAPPROPRIATETRANSFIRECOST;
			parameters[ParamSettingExtendedCostBasedReplay.ALLOWINAPPROPRIATEFIRING] = false;
			parameters[ParamSettingExtendedCostBasedReplay.REPLAYEDEVENTCOST] = DEFREPLAYEDEVENTCOST;
			parameters[ParamSettingExtendedCostBasedReplay.HEURISTICDISTANCECOST] = DEFREPLAYEDEVENTCOST;

//			parameters[ParamSettingExtendedCostBasedReplay.SKIPPEDEVENTCOST] = basIdentifyExtraEvts.isSelected() ? DEFSKIPPEDEVENTCOST
//					: 0;
			parameters[ParamSettingExtendedCostBasedReplay.SKIPPEDEVENTCOST] = DEFSKIPPEDEVENTCOST;
//			parameters[ParamSettingExtendedCostBasedReplay.ALLOWEVENTSKIP] = basIdentifyExtraEvts.isSelected();
			parameters[ParamSettingExtendedCostBasedReplay.ALLOWEVENTSKIP] = true;

			parameters[ParamSettingExtendedCostBasedReplay.INITIATIVEINVISTASKCOST] = basIdentifyInvi.isSelected() ? DEFINITIATIVEINVISTASKCOST
					: 0;
			parameters[ParamSettingExtendedCostBasedReplay.ALLOWINVITASK] = basIdentifyInvi.isSelected();

			parameters[ParamSettingExtendedCostBasedReplay.INITIATIVEREALTASKCOST] = yesExecRealTask.isSelected() ? DEFINITIATIVEREALTASKCOST
					: 0;
			parameters[ParamSettingExtendedCostBasedReplay.ALLOWREALTASK] = basIdentifyReal.isSelected();

			parameters[ParamSettingExtendedCostBasedReplay.MAXEXPLOREDINSTANCESINTVAL] = basMaxInstance.isSelected() ? limMaxEvents
					.getValue()
					: MAXLIMMAXNUMINSTANCES;

			parameters[ParamSettingExtendedCostBasedReplay.ALLOWTASKEXECWOTOKENS] = false;

		} else {
			parameters[ParamSettingExtendedCostBasedReplay.ALLOWINAPPROPRIATEFIRING] = yesInappropriateFiring.isSelected();
			parameters[ParamSettingExtendedCostBasedReplay.INAPPROPRIATETRANSFIRECOST] = yesInappropriateFiring.isSelected() ? unsatisfiedEventsSlider
					.getValue()
					: 0;
			...
			parameters[ParamSettingExtendedCostBasedReplay.SKIPPEDEVENTCOST] = yesSkipEvent.isSelected() ? skipEventSlider
					.getValue() : 0;
			parameters[ParamSettingExtendedCostBasedReplay.ALLOWEVENTSKIP] = yesSkipEvent.isSelected();

			parameters[ParamSettingExtendedCostBasedReplay.INITIATIVEINVISTASKCOST] = yesExecInviTask.isSelected() ? execInviTaskSlider
					.getValue()
					: 0;
			parameters[ParamSettingExtendedCostBasedReplay.ALLOWINVITASK] = yesExecInviTask.isSelected();

			parameters[ParamSettingExtendedCostBasedReplay.INITIATIVEREALTASKCOST] = yesExecRealTask.isSelected() ? execRealTaskSlider
					.getValue()
					: 0;
			parameters[ParamSettingExtendedCostBasedReplay.ALLOWREALTASK] = yesExecRealTask.isSelected();

			parameters[ParamSettingExtendedCostBasedReplay.MAXEXPLOREDINSTANCESINTVAL] = maxExpInstSlider.getValue();

			parameters[ParamSettingExtendedCostBasedReplay.ALLOWTASKEXECWOTOKENS] = false;
		}

		parameters[ParamSettingExtendedCostBasedReplay.ISTESTINGMODEINTVAL] = isTestingMode;
		return parameters;
	}

	@Override
	public Object getParameterValue(int paramVariableValIndex) {
		if (paramVariableValIndex == ParamSettingExtendedCostBasedReplay.MAXEXPLOREDINSTANCESINTVAL) {
			if (useBasic) {
