...
import org.processmining.plugins.fuzzymodel.miner.util.ColorRepository;
...
public class UnaryMetricsPanel extends JComponent implements MouseListener, MouseMotionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4929867347657997407L;
	...
	public UnaryMetricsPanel(FMLogEvents events) {
		this.events = events;
	...
	protected void paintComponent(Graphics g) {
		...
		for(int i=0; i<colors.length; i++) {
			colors[i] = ColorRepository.getGradualColor(i / ((double)colors.length - 1));
		}
		// prepare drawing
		...
		if(width > measurePoints) {
			...
			for(int i=0; i<metrics.size(); i++) {
				...
				int x = 0;
				int lastY = 5 + chartHeight - (int)(metric.getMeasure(0) * chartHeight);
				for(int k=1; k<measurePoints; k++) {
					int y = 5 + chartHeight - (int)(metric.getMeasure(k) * chartHeight);
			...
			for(int i=0; i<metrics.size(); i++) {
				...
				int lastY = 5 + chartHeight - (int)(aggregateValues(0, clusterSize, metric) * chartHeight);
				for(int x=1; x<actualWidth; x++) {
					int y = 5 + chartHeight - (int)(aggregateValues(x, clusterSize, metric) * chartHeight);
	...
	//	String lineB = "(" + events.get(eventIndex).getEventType() + ")";
		XEvent evt = events.get(eventIndex);
		String lineA  = FMLog.getConceptName(evt);
		String lineB = "(" + FMLog.getLifecycleTransition(evt) + ")";
