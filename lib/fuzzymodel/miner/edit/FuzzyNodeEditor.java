...
public class FuzzyNodeEditor extends JPanel {
	...
	protected ProMGraphCell graphNode;
	protected PluginContext context;
	protected FMNode fuzzyNode;
	...
	public FuzzyNodeEditor(PluginContext context, FuzzyGraphEditorPanel parent, ProMGraphCell node,
			ActionListener closeListener) {
		this.context = context;
		this.parent = parent;
		this.graphNode = node;
		this.graph = parent.graphForEdit;
		this.fuzzyNode = (FMNode) node.getNode();
		isCluster = (this.fuzzyNode instanceof FMClusterNode);
	...
	protected void initializeGui() {
		...
		JPanel namePanel = FuzzyGraphEditorPanel.layoutHorizontally(new Component[] { nameLabel,
				Box.createHorizontalStrut(5), nameField }, 20);
		// node type
		...
		JPanel typePanel = FuzzyGraphEditorPanel.layoutHorizontally(new Component[] { typeLabel,
				Box.createHorizontalStrut(5), typeField }, 20);
		// significance slider
		significanceSlider = new NiceDoubleSlider("significance", 0, 1.0, fuzzyNode.getSignificance(),
		...
		if (isCluster) {
			...
			significanceSlider.addChangeListener(new ChangeListener() {
				public void stateChanged(ChangeEvent e) {
					// live update of node property changes
					System.out.println("To set the significance is " + significanceSlider.getValue());
					fuzzyNode.setSignificance(significanceSlider.getValue());
					System.out.println("After setting the Significance is " + fuzzyNode.getSignificance());
					updateGraphNodeLabel();
				}
			});
		}
		...
		deleteNodeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				graph.removeNode(fuzzyNode);
			//	graph.hidePermanently(fuzzyNode);
		...
		if (isCluster == true) {
			...
			String warning = "<html>You can only edit the name of a cluster node, or "
					...
					+ "Editing event type and significance is reserved for primitive " + "nodes.</html>";
	...
	protected void updateGraphNodeLabel() {
		 ...
		 * graphNode.updateView(); graphNode.updateView();
		 */
		/*
		 * Map<Object,Object> nested = new HashMap<Object,Object>();
		 * Map<Object,Object> attributeMap1 = new HashMap<Object,Object>();
		 * GraphConstants.setValue(attributeMap1, fuzzyNode.getLabel());
		 * nested.put(graphNode, attributeMap1); ProMJGraph jGraph =
		 * this.parent.graphPanel.getGraph();
		 * jGraph.getGraphLayoutCache().edit(nested,null,null,null);
		 */
		String label = "";
		if (fuzzyNode instanceof FMClusterNode) {
			FMClusterNode clusterNode = (FMClusterNode) fuzzyNode;
			label = "<html>" + clusterNode.getElementName() + "<br>" + Integer.toString(clusterNode.size())
					+ " elements " + "<br>" + "~" + MutableFuzzyGraph.format(clusterNode.getSignificance()) + "<html>";
		} else {

			label = "<html>" + fuzzyNode.getElementName() + "<br>" + fuzzyNode.getEventType() + "<br>"
					+ MutableFuzzyGraph.format(fuzzyNode.getSignificance()) + "<html>";
		}

		fuzzyNode.getAttributeMap().put(AttributeMap.LABEL, label);
	}
