 ...
 * 
 */
@Plugin(name = "ILP Miner", categories = { PluginCategory.Discovery }, parameterLabels = { "Log", "Log Info", "Settings" }, returnLabels = {
		"Petrinet", "Marking" }, returnTypes = { Petrinet.class, Marking.class })
public class ILPMiner {
	 ...
	 * @throws Exception
	 */
	@UITopiaVariant(uiLabel = "Mine for a Petri Net using ILP", affiliation = UITopiaVariant.EHV, author = "T. van der Wiel", email = "t.v.d.wiel@student.tue.nl")
...
//		}
	 ...
	 * @throws Exception
	 */
	@PluginVariant(variantLabel = "With settings", requiredParameterLabels = {
			0, 1, 2 })
	 ...
	 * @throws Exception
	 */
	private Object[] doILPMiningPrivateWithRelations(PluginContext context,
	...
	private Object[] doILPMiningPrivateWithRelationsAndLanguage(
		...
		case CPLEX:
			...
			try {
				Constructor<?> mc = settings.getVariant().getConstructor(
						new Class[] { IloOplFactory.class, Class[].class,
								Map.class, ILPModelSettings.class });
				modelCPLEX = (ILPModelCPLEX) mc.newInstance(new Object[] {
						oplF, settings.getExtensions(),
		...
		default: // JAVAILP_XXX
			...
			try {
				...
				modelJavaILP = (ILPModelJavaILP) mc.newInstance(new Object[] {
						settings.getExtensions(), settings.getSolverSettings(),
						settings.getModelSettings() });
			} catch (Exception e) {
				throw e;
			}
