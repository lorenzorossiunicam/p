 ...
 * 
 */
@ILPMinerStrategy(description = "<p>Generates a net by solving the relaxed problem and round the solution</p><p>to the nearest integers.</p>", name = "Petri Net (Relaxed)")
public class PetriNetLPModel extends PetriNetILPModel {
	protected boolean causalDependencies;

	public PetriNetLPModel(Class<?>[] extensions, Map<SolverSetting, Object> solverSettings, ILPModelSettings settings) {
	...
	protected ILPMinerSolution makeSolution(Result result) {
		...
		for (int t = 0; t < trans; t++) {
			x[t] = Math.round(result.get("x" + t).doubleValue());
			y[t] = Math.round(result.get("y" + t).doubleValue());
		}
		double c = Math.round(result.get("c").doubleValue());
